...
public:
	...
	void displayrev();										//wyswietla listeb w odwrotnej kolejnoci
	int length();											//wyswietla dlugosc listy
	const list1 operator+(list1 & rhs) const;		//dodawanie list
	const list1 operator-(list1 & rhs) const;		//odejmowanie list
	const list1 operator=(const list1 & rhs);		//kopiowanie list
	void deleteduplicate();									//usuwa powtarzajace sie elementy
	const list1 operator+=(list1 & rhs);				//dodaje do listy
	const list1 operator-=(const list1 & list3);		//odejmuje od listy
	int valuebyindex(int i);								//zwraca wartosc elemtnu w danym miejscu listy
	const bool operator<(list1 & rhs);				
	const bool operator<=(list1 & rhs);
	const bool operator>(list1 & rhs);
	const bool operator>=(list1 & rhs);
	const bool operator==(list1 & rhs);
	...
	node* operator[](int n);							//zwraca wskaznik na element na danym miejscu
